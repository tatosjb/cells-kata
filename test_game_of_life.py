...
def test_living_cell_with_no_neighbors_dies():
    ...
    assert game.run_and_get_state()[1][1] == 0
    assert game.run_and_get_state() == [
        [0, 0, 0],
        [0, 0, 0],
        [0, 0, 0],
...
def test_a_living_cell_with_two_neighbors_survives():
    ...
    assert game.run_and_get_state()[1][1] == 1
...
def test_two_iterations():
    ...
    assert game.run_and_get_state(2) == [
        [0, 1, 0],
        [0, 1, 0],
        [0, 1, 0],
    ]
...
def test_three_iterations():
    """Test that the game state is correctly updated after three iterations."""
    game = GameOfLife(
        [
            [0, 1, 0],
            [0, 1, 0],
    ...
    assert game.run_and_get_state(3) == [
        [0, 0, 0],
        [1, 1, 1],
        [0, 0, 0],
    ]
...
def test_2x3_grid():
    """Test that the game state is correctly updated for a 2x3 grid."""
    game = GameOfLife(
        [
            [0, 1],
            [0, 1],
