...
class GameOfLife:
    ...
    def run_and_get_state(self, iterations: int = 1):
        """Run the game for a given number of iterations and return the final state of the grid."""
        for _ in range(iterations):
            self.grid = self._run_and_get_next_state()
        return self.grid

    def _run_and_get_next_state(self) -> List[List[int]]:
